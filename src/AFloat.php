...
class AFloat
{
    ...
    {
        return (static::is($value) || (is_numeric($value) && !AInt::extIs($value)));
    }
    ...
    public static function random($min = 0.0, $max = 1.0, $round = null, $seed = null)
    {
        ...
        !is_null($seed) && AInt::is($seed) && mt_srand($seed);

        // Ensure that max is not inclusive
        $rand = $min + (((mt_rand() - 1) / mt_getrandmax()) * abs($max - $min));

        // Round if needed
        (!is_null($round) && AInt::is($round) && $round > 0) && ($rand = round($rand, $round));
