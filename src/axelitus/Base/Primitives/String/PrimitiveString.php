...
abstract class PrimitiveString extends Primitive
{
     ...
     *
     * @param string|PrimitiveString $input    The input string.
     * @param int|PrimitiveInt       $start    The start index from where to begin extracting.
     * @param int|PrimitiveInt       $length   The length of the extracted substring.
     * @param string|PrimitiveString $encoding The encoding of the $input string for multibyte functions.
     *
     * @throws \InvalidArgumentException
    ...
    public static function sub($input, $start, $length = null, $encoding = self::DEFAULT_ENCODING)
    {
        ...
        try {
            ...
            $length = is_null($length) ? (function_exists('mb_substr') ? mb_strlen($input, $encoding) : strlen(
                    $input
                )) - $start : Int::native($length);
        } catch (\InvalidArgumentException $ex) {
            throw new \InvalidArgumentException("The \$start and \$length parameters must be integers or instances derived from PrimitiveInt.");
        }
     ...
     * @param string|PrimitiveString $encoding       The encoding of the input string
     * @param null|int               $count          If passed, this will be set to the number of replacements performed.
     *
     * @throws \InvalidArgumentException
     ...
     * @param   int|PrimitiveInt       $limit        The number of characters to truncate too
    ...
    public static function truncate($input, $limit, $continuation = '...', $is_html = false)
    {
        ...
        try {
            ...
            throw new \InvalidArgumentException("The \$limit parameter must be an integer or instance derived from PrimitiveInt.");
        }
     ...
     *
     * @param   int|PrimitiveInt       $length  The length of the output string
    ...
    public static function random($length = 1, $chars = self::ALNUM, $shuffle = false)
    {
        try {
            ...
            throw new \InvalidArgumentException("The \$length parameter must be an integer or instance derived from PrimitiveInt.");
        }
     ...
     * @param int|PrimitiveInt       $flags   The flag modifiers.
     * @param int|PrimitiveInt       $offset  The offset from which to start the search (in bytes).
     *
     * @throws \InvalidArgumentException
     * @return int
    ...
    public static function match($input, $pattern, array &$matches = null, $flags = 0, $offset = 0)
    {
        ...
        try {
            ...
            throw new \InvalidArgumentException("The \$flags and \$offset parameters must be integers or instances derived from PrimitiveInt.");
        }
