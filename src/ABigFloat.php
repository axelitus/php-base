...
class ABigFloat
{
    ...
    {
        return (is_float($value) || (is_numeric($value) && !is_int($value) && AStr::contains($value, '.')));
    }
    ...
    public static function compare($float1, $float2, $scale = null)
    {
        ...
        if (is_float($float1) && is_float($float2)) {
            ...
            return ABigNum::toFloat(((is_null($scale))
                ? bcsub($float1, $float2)
     ...
     *
     * It's an alias for ABigFloat::inRange($value, $lower, $upper, false, false)
     *
     * @param float|string $value The value to test in range.
     * @param float|string $lower The range's lower limit.
     * @param float|string $upper The range's upper limit.
     ...
     *
     * It's an alias for ABigFloat::inRange($value, $lower, $upper, true, true)
     *
     * @param float|string $value The value to test in range.
     * @param float|string $lower The range's lower limit.
     * @param float|string $upper The range's upper limit.
    ...
    public static function abs($float)
    {
        ...
        if (AFloat::is($float)) {
            return AFloat::abs($float);
        } else {
            return AStr::replace($float, '-', '');
        }
    }
    ...
    public static function mod($base, $modulus, $scale = null)
    {
        ...
        if (is_float($base) && is_float($modulus)) {
            ...
            $times = ABigNum::toFloat(((is_null($scale))
                ? static::toInt(ABigNum::toFloat(bcdiv($base, $modulus)))
                : static::toInt(ABigNum::toFloat(bcdiv($base, $modulus, $scale)))
            ));

            return ABigNum::toFloat(static::sub($base, static::mul($times, $modulus, $scale), $scale));
        }
