...
/**
 * Class ABigFloatComparator
 *
 * BigFloat comparator implementation.
 *
 * @package axelitus\Base
 * @SuppressWarnings(PHPMD.StaticAccess)
 */
class ABigFloatComparator extends Comparator
{
    /**
     * Constructor
     *
    ...
    public function __construct($scale = null)
    {
        parent::__construct(
            function ($item1, $item2) {
                if (!ABigFloat::is($item1) || !ABigFloat::is($item2)) {
                    throw new \InvalidArgumentException(
                        "The \$item1 and \$item2 parameters must be of type float (or string representing a big float)."
                    );
                }

                return ABigFloat::compare($item1, $item2, $this->options['scale']);
            }
        );
